...
public class DinoAIEatItems extends EntityAIBase {
    ...
    public boolean shouldExecute() {
        if (this.prehistoric.getHunger() >= this.prehistoric.getMaxHunger()) {
            ...
            return false;
        } else if (this.prehistoric.getRNG().nextInt(1) != 0) {
            return false;
        }
    ...
    private EntityItem getNearestItem(int range) {
        List<EntityItem> nearbyItems = this.prehistoric.world.getEntitiesWithinAABB(EntityItem.class, this.prehistoric.getEntityBoundingBox().expand(range, range, range));
        ...
        for (EntityItem currentItem : nearbyItems) {
            if (!currentItem.getItem().isEmpty()) {
                ...
                if (foodValue > 0 && this.prehistoric.getDistanceSq(currentItem) < range * range) {
                    return currentItem;
                }
            }
        }
